...
impl Interpreter {
    ...
    pub fn built_int_function_handler(&mut self, function_token: Token) {
        match &*function_token.value {
            ...
            "swap" => self.swap(self.current_instruction.x, self.current_instruction.y),
            "rotate" => self.rotate(self.current_instruction.x, self.current_instruction.y),
            "drop" => self.drop(self.current_instruction.x, self.current_instruction.y),
    ...
    fn _close_scope(&mut self, error: String, scope_type: &str) {
        ...
        loop {
            ...
            match self.current_instruction.token_type {
                ...
                TokenType::EndOfFile => self.error(
                    error.clone(),
    ...
    fn close_if(&mut self) {
        ...
        loop {
            ...
            match self.current_instruction.token_type {
                ...
                TokenType::EndOfFile => self.error(
                    "Error: Unexpected End for conditionals".to_string(),
                    self.current_instruction.x,
                    self.current_instruction.y,
                    "if".to_string(),
                ),
    ...
    pub fn run(&mut self) {
        loop {
            ...
            if self.current_instruction.is_data_type() {
                match self.current_instruction.token_type {
                    TokenType::Integer => self.push_main(Value::new_int(
                        self.current_instruction.value.parse::<i128>().unwrap(),
                    )),
                    TokenType::FloatingPoint => self.push_main(Value::new_float(
                        self.current_instruction.value.parse::<f64>().unwrap(),
                    )),
                    TokenType::String => {
                        self.push_main(Value::new_string(self.current_instruction.value.clone()))
                    }
                    TokenType::Boolean => self.push_main(Value::new_bool(
                        self.current_instruction.value.parse::<bool>().unwrap(),
                    )),
                    _ => panic!("weird error"),
                }
            } else {
                match self.current_instruction.token_type {
                    TokenType::MathOperation => self.math_handler(self.current_instruction.clone()),
                    TokenType::Identifier => {
                        if self
                            .built_in_functions
                            .contains(&self.current_instruction.value)
                        {
                            self.built_int_function_handler(self.current_instruction.clone())
                        } else if self
                            .defined_functions
                            .contains_key(&self.current_instruction.value)
                        {
                            self.end_users.push("inst".to_string());
                            self.calls.push(self.index as u64);
                            self.move_to(
                                self.defined_functions
                                    .get(&self.current_instruction.value)
                                    .unwrap()
                                    .clone() as i64,
                            )
                        } else {
                            if self.index == self.instructions.len() as i64 {
                                self.error(
                                    "Error: Unexpected Identifier".to_string(),
                                    self.current_instruction.x,
                                    self.current_instruction.y,
                                    "".to_string(),
                                )
                    ...
                    TokenType::If => {
                        ...
                        if condition.val_type != ValueType::Boolean {
                            self.error(
                                "Error: Conditions can only accept Booleans".to_string(),
                                self.current_instruction.x,
                                self.current_instruction.y,
                                "if".to_string(),
                            )
                    ...
                    TokenType::Loop => {
                        ...
                        self.loops
                            .push((loop_times.val_int as u64, self.index as u64))
                    }
                    ...
                    TokenType::Else => {
                        if self.conditional_scopes < 1 {
                            self.error(
                                "Error: Unexpected Else Block".to_string(),
                                self.current_instruction.x,
                                self.current_instruction.y,
                                "end".to_string(),
                            )
                        ...
                        while scopes != 0 {
                            ...
                            match self.current_instruction.token_type {
                                ...
                                TokenType::EndOfFile => self.error(
                                    "Error: Expected Continuation of conditionals".to_string(),
                                    self.current_instruction.x,
                                    self.current_instruction.y,
                                    "if".to_string(),
                                ),
                    ...
                    TokenType::Break => {
                        if self.loops.len() == 0 {
                            self.error(
                                "Error: Unexpected Break, not in a loop".to_string(),
                                self.current_instruction.x,
                                self.current_instruction.y,
                                "break".to_string(),
                            )
        ...
        if self.conditional_scopes > 0 {
            self.error(
                "Error: Unclosed Block".to_string(),
                self.current_instruction.x,
                self.current_instruction.y,
                "EndProgram".to_string(),
            );
        }
    }
}
